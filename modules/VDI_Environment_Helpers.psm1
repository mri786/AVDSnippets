...
function Get-MultiEnvNames {
    ...
    $environmentNames = [PSCustomObject]@{
        ...
        fslogixUserGroup                 = 'UG_{0}_AAD_VDI_HPL_MULT_{1}_{2}' -f $msgEnv.ToUpper(), $BusinessUnitId, $HostPoolId
...
function Build-DeploymentFile {
    ...
    if (Select-String -Path $FilePath -Pattern "#{variables..+}#" -Quiet) {
        Write-Host "##[error]❗`t$($functionName): The following patterns were missed:"
